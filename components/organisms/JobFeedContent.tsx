      ...
      <div className="mx-auto w-full max-w-7xl px-6 lg:px-8">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {loading ? (
              <div className="bg-white rounded-3xl border border-[#ece7df] p-10 text-center text-[#6b6560]">
                Loading jobs...
              </div>
            ) : errorMessage ? (
              ...
              filteredJobs.map((job) => <JobFeedCard key={job.id} job={job} />)
            ) : (
...
function JobFeedCard({ job }: { job: JobFeedItem }) {
  const router = useRouter();

  return (
    <div className="bg-white rounded-3xl border border-[#ece7df] p-6 shadow-sm transition-all hover:shadow-xl hover:border-[#F7931A]/30 group overflow-hidden">
      {/* Top row */}
      ...
      <p className="text-sm text-[#6b6560] leading-relaxed mb-4 overflow-hidden break-all" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
  {job.description}
</p>
