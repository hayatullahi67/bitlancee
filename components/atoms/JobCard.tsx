...
export default function JobCard({ icon, title, description, price, tags, variant = 'default', titleClassName, descriptionClassName, priceClassName, tagsClassName, isFreelancer = false, profileHref }: JobCardProps) {
  if (variant === 'findwork') {
    return (
      <div className="min-w-0 overflow-hidden rounded-[34px] bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
        <div className="mb-3 flex items-start justify-between gap-3 min-w-0">
          ...
          <div className={`shrink-0 text-[20px] font-bold text-[#8C4F00] ${priceClassName || ''}`}>{price}</div>
        </div>

        <div className="mb-3 flex min-w-0 items-start justify-between gap-3">
          <h4 className={`min-w-0 flex-1 break-words text-xl font-semibold text-[#1a1a1a] ${titleClassName || ''}`}>{title}</h4>
          {!isFreelancer && <span className="shrink-0 text-xs font-bold uppercase text-[#554335]">Fixed</span>}
        </div>

        <p className={`mb-4 break-words text-sm leading-6 text-[#555] ${descriptionClassName || ''}`}>{description}</p>
        ...
        {!isFreelancer && (
          <div className="flex justify-end">
            <button className="rounded-full bg-[#F7931A] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#a15f00]">
              Apply Now
            </button>
          </div>
        )}
        {isFreelancer && (
          <div className="flex justify-end">
            {profileHref ? (
              <Link
                href={profileHref}
                className="rounded-full bg-[#F7931A] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#a15f00]"
              >
                View Profile
              </Link>
            ) : (
